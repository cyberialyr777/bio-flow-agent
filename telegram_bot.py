import os
import re  # <-- NUEVO IMPORT PARA EL FORMATO
import logging
from datetime import datetime
from dotenv import load_dotenv
from strands import Agent

# Importamos las herramientas de tu archivo tools.py
from tools import (
...
from telegram.constants import ChatAction, ParseMode  # <-- NUEVO IMPORT (ParseMode)
...
# --- CONFIGURACIÓN DEL AGENTE BIO-FLOW ---
...
# --- FORMATEADOR PARA TELEGRAM ---
def format_for_telegram(text: str) -> str:
    """Convierte el Markdown de Claude a HTML amigable para Telegram."""
    # 1. Elimina etiquetas ocultas (como <thinking>) para que no rompan Telegram
    text = re.sub(r'<thinking>.*?</thinking>', '', text, flags=re.DOTALL)
    
    # 2. Convierte los asteriscos dobles (**texto**) a negritas de HTML (<b>texto</b>)
    text = re.sub(r'\*\*(.*?)\*\*', r'<b>\1</b>', text)
    
    # 3. Convierte las viñetas feas de asteriscos (* Item) a puntos elegantes (• Item)
